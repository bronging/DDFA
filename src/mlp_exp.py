...
from preprompt import PrePrompt,  pca_compression, PrePromptNorm
import preprompt
import pdb
import os
import sys
...
try:
    ...
    if model_type == 'samgpt' or model_type == 'vec2mlp' or model_type == 'norm_mdgpt':
        if pretrain_method == 'GRAPHCL':
            ...
            train.train_samgpt_graphcl(model=model, lr=lr, weight_decay=l2_coef, 
                            ...
                            lbls=lbls, sparse=sparse, save_name=save_name, patience=patience, pretrain_dataset_names=pretrain_dataset_names)
        ...
        train.train_anchor_mlp_graphcl_graphbat(model, dataloader, args.restrart_epoch, nb_epochs, \
                    save_name, sparse, args.separate_learning, LP, patience=50, \
                    w1alpha=1, lr=0.001, weight_decay=0.0, )

from utils.figure import plot_similarity_heatmap
# pretrain_loaders = [DataLoader(load_dataset(dataset)) for dataset in pretrain_dataset_names]           
# features, adjs, aug_features, aug_adjs, \
#         lbls, negetive_samples, combinedadj = process.preprocess_dataset(
#                                                 pretrain_loaders, pretrain_method, 
...
# # PCA 적용한 피처 w1 distance 
# features = [tensors.cuda() for tensors in features]
# plot_similarity_heatmap(features, pretrain_dataset_names, downstream)

# # 축소된 피처 w1 distance heatmap 그리기 
# arr = model.get_token(aug_features) # GCN 통과 전     
# plot_similarity_heatmap(arr, pretrain_dataset_names, downstream)

# # 임베딩의 w1 distane heatmap 
# arr = [] 

...
if model_type == 'GraphACL': 
    ...
    model = PrePromptACL(unify_dim, hid_units, num_pretrain_dataset_num, num_layers_num, 
    ...
    write(f'   de loss         : {args.de_loss}')
    write(f'   gamma           : {args.gamma}')
    model = PrePromptSharedFUG(unify_dim, hid_units, nonlinearity, num_pretrain_dataset_num, 
            num_layers_num, 0.1, type_ = args.combinetype, backbone = args.backbone, #'norm_mdgpt',
            alpha = args.alpha, ablation = ablation_pre, scaling_factor=3, n_sample=sample_size, 
            if_rand=False,  de_loss=args.de_loss, de_weight=args.de_weight, sampling=args.sampling,
    ...
    write(f'   gamma           : {args.gamma}')
    write(f'   shared          : {args.shared}')
    model = PrePromptFilterFUG(unify_dim, hid_units, nonlinearity, num_pretrain_dataset_num, 
            num_layers_num, 0.1, type_ = args.combinetype, backbone = args.backbone, #'norm_mdgpt',
            alpha = args.alpha, ablation = ablation_pre, scaling_factor=3, n_sample=sample_size, 
            if_rand=False,  de_loss=args.de_loss, de_weight=args.de_weight, sampling=args.sampling,
            n_mlp_layer=args.n_mlp_layer, de_input=args.de_input, shared=args.shared).cuda()
    ...
    write(f'   gamma           : {args.gamma}')
    write(f'   shared          : {args.shared}')
    model = FilterbankFUG(in_dim=unify_dim, hid_dim=hid_units, activation=nonlinearity, num_pretrain_dataset=num_pretrain_dataset_num, 
                          gcn_layers=num_layers_num, dropout=0.1, type_=args.combinetype,  alpha=args.alpha, ablation=ablation_pre, 
...
try:
    ...
    if model_type in ['samgpt', 'norm_mdgpt', 'sharedFUG', 'filterFUG', 'filterbank']:
        if pretrain_method == 'GRAPHCL':
            ...
            if args.ep_aug: 
                ...
                train.train_fug_graphcl(model=model, lr=lr, weight_decay=l2_coef, 
                        ...
                        lbls=lbls, sparse=sparse, save_name=save_name, patience=patience, gamma=args.gamma, model_type=model_type)

...
class downstreampromptFilterFUG(nn.Module):
    ...
    def forward(self, seq, gcn, adj, sparse):
        ...
        t_low = self.low_dim(sample[:, low_freq_indices])
        # H_low = self.low_dim.feature_sig_propagate(seq[:, low_freq_indices], t_low)
        ...
        seq6 = torch.cat([H_low, (H_high+H_composed_high_X)], dim=1)

        if self.shared: 
            seq1 = self.shared_token * F.relu(seq1)
            seq2 = self.shared_token * F.relu(seq2)
            seq3 = self.shared_token * F.relu(seq3)

            seq4 = self.shared_token * F.relu(seq4)
            seq5 = self.shared_token * F.relu(seq5)
            seq6 = self.shared_token * F.relu(seq6)
        # balance token 

...
import tqdm
import argparse
from downprompt import downprompt, downprompt_graph, TargetAlignedModel, PrototypeClassifier, downprompt2
import csv
...
if model_type == 'samgpt' or  model_type == 'vec2mlp':
    if pretrain_method == 'GRAPHCL':
        #Vec2MLP
        for i, prompt in enumerate(model.feature_prompt_layers):
            for layer in prompt.net:
                if isinstance(layer, torch.nn.Linear):
                    print(f'Domain {i} feature prompt weight shape: {layer.weight.shape}')
                    print(layer.weight)
        ...
        for i in range(5):
            ...
            for j in range(i+1, 6): 
                w1 = sliced_wasserstein_torch(aug_features[i][0], aug_features[j][0])
                a.append(w1)
            feat_w1.append(a)
        arr_graph = model.get_token(aug_features)
        len(arr_graph)
        ...
        for feat, adj, graph in zip(aug_features, aug_adjs, arr_graph): 
            emb =  model.embed(feat[0], adj[0], sparse, None, LP)[0].squeeze(0)
            emb2 = model.embed(graph, adj[0], sparse, None, LP)[0].squeeze(0)
            
            raw_emb.append(emb)
            token_emb.append(emb2)

            w2 = sliced_wasserstein_torch(emb, emb2)
            w1_emb.append(w2)
        ...
        for i in range(5):
            ...
            c = []
            for j in range(i+1, 6): 
                w1 = sliced_wasserstein_torch(arr_graph[i], arr_graph[j])
                a.append(w1)
                w3 = sliced_wasserstein_torch(raw_emb[i], raw_emb[j])
                w4 = sliced_wasserstein_torch(token_emb[i], token_emb[j])
                b.append(w3)
                c.append(w4)
        ...
        for i in range(5): 
            for j in range(i+1, 6): 
                print(f'[{data[i]}] - [{data[j]}] W1 =========')
                print(f'Before: {feat_w1[i][j-i-1]:.6f}') # 원래 피처 간의 W1
                print(f'After : {w1_arr[i][j-i-1]:.6f}\n') # 도메인 토큰 적용한 피처 간의 W1
                print(f'Before emb: {w3_emb_arr[i][j-i-1]:.6f}') # GNN (original feat) 간의 W1
                print(f'After emb : {w4_emb_arr[i][j-i-1]:.6f}\n') # GNN (token * feat) 간의 W1 

        torch.save(w3_emb_arr, f'{save_name}_before_emb.ptr')
        torch.save(w4_emb_arr, f'{save_name}_after_emb')

elif model_type == 'permMDGPT': 
        if args.separate_learning == True: # GCN 튜닝 -> MLP 튜닝  GraphCL loss + MLP loss 
            for i, prompt in enumerate(model.feature_prompt_layers):
                print(f'Domain {i} feature prompt weight shape: {prompt.weight.shape}')
                print(prompt.weight)
...
if model_type == 'samgpt': 
    ...
    token_em = log.get_emb(features, model.gcn, adj, sparse).squeeze(0)
    for i in range(6): 
        w1dist = sliced_wasserstein_torch(emb, token_emb[i])
        w2dist = sliced_wasserstein_torch(token_em, token_emb[i])
        w.append(w1dist)
        w2.append(w2dist)
    
    for i in range(6): 
        print(f'[{data[args.target_id]}] - [{data[i]}] W1 ====')
        print(f"Before unifying: {w[i]}")
        print(f"After unifying : {w2[i]}\n")
    
    torch.save(w, f'{save_path}_before_unifying.ptr')
    torch.save(w2, f'{save_path}_after_unifying.ptr')

    
    print(f'unifying token: {log.downstreamPrompt.open_prompt_fea.weight.shape}')
    print(f'{log.downstreamPrompt.open_prompt_fea.weight}')


elif model_type == 'vec2mlp':

    fea_pretext_weights, combines = model.get_weights()
    combines.append(args.beta)

    log = downprompt2(hid_units, nb_classes, unify_dim,
                fea_pretext_weights, combines, args.combinetype,
                ablation_down, n_mlp_layer=n_mlp_layer, 
                init_identity=args.mlp_init, mlp_bias=mlp_bias, 
                mixing_mlp=mixing_mlp).cuda()
    log.load_state_dict(torch.load(save_path))

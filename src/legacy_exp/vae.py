...
try:
    ...
    # for i in range(len(features)): 
        # features[i] = torch.FloatTensor(pca_compression(features[i], k=None)).cuda() # 원래 차원으로 축소 
    gamma = 1
    # feature compression 
    vase, features = train_vaes(features, hidden_dim=256, k=unify_dim, beta=1e-3, gamma=gamma,  epochs=nb_epochs)
    print(len(features))

    # features_list = features
    # num_nodes_list = [feat.shape[0] for feat in features_list]  # e.g. [50, 60, 45]
    # concat_features = torch.cat(features_list, dim=0)  # Tensor[num_total_nodes, d]

    # pre_vae_model, encoded_features  = train_single_vae(concat_features, hidden_dim=256, k=unify_dim, epochs=nb_epochs)
    # # 3. Split back into per-graph feature matrices
    # split_features = torch.split(encoded_features, num_nodes_list, dim=0)
    # # split_features: Tuple[Tensor[num_nodes_i, k]], same order as original
    # features = list(split_features)
...
# vae_model, t_feature = train_single_vae(t_feature, hidden_dim=256, k=unify_dim, epochs=nb_epochs)
# with torch.no_grad():
#     _, _, _, t_feature = pre_vae_model(t_feature) 

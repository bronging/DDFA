...
from preprompt import PrePrompt, PrePromptNorm, pca_compression
import preprompt
import pdb
import os
import sys
...
if args.target_id == 0: 
    pretrain_dataset_names = ['Computers', 'FacebookPagePage']
...
if model_type == 'samgpt': 
    ...
    model = PrePromptNorm(unify_dim, hid_units, nonlinearity, num_pretrain_dataset_num, 
            num_layers_num, 0.1, type_ = args.combinetype, backbone = args.backbone,
            alpha = args.alpha, ablation = ablation_pre).cuda()
    best_model = PrePromptNorm(unify_dim, hid_units, nonlinearity, num_pretrain_dataset_num, 
            num_layers_num, 0.1, type_ = args.combinetype, backbone = args.backbone,
            alpha = args.alpha, ablation = ablation_pre).cuda()
    worst_model = PrePromptNorm(unify_dim, hid_units, nonlinearity, num_pretrain_dataset_num, 
            num_layers_num, 0.1, type_ = args.combinetype, backbone = args.backbone,
            alpha = args.alpha, ablation = ablation_pre).cuda()
...
try:
    ...
    if torch.cuda.is_available():
        ...
        pre_features = [tensors.cuda() for tensors in features]
        ...
        if args.pretrain_method == 'LP':
            negetive_samples = negetive_samples.cuda()
        else: 
    ...
    if model_type == 'samgpt' or model_type == 'norm_mdgpt':
        ...
        for n in range(num_permutations): 
            ...
            idx_perm = np.random.permutation(unify_dim)
            perm_list.append(idx_perm)
    

            if args.pretrain_method == 'LP':
                print(f'********feature size: {len(pre_features)}, {pre_features[0].size()}')
                feat = pre_features[1]   
                pre_features[1] = feat[:, idx_perm]
                perm_list.append(idx_perm)
                print(f'[{n}]: {idx_perm}')
            else: 
                perm = raw_feature
                aug_features[1] = perm[:, :, idx_perm]
                perm_list.append(idx_perm)
                print(f'[{n}]: {idx_perm}')
            ...
            if args.pretrain_method == 'LP' or args.pretrain_method == 'splitLP':
                train.train_samgpt_lp(model=model, lr=lr, weight_decay=l2_coef, 
                            start_epoch=args.restart_epoch, num_epoch=nb_epochs, 
                            features=pre_features, adjs=adjs, negetive_samples=negetive_samples,
                            sparse=sparse, save_name=save_name, patience=patience)
                            
            if ablation_down == 'None': 
                log, acc = adapation.adaptation_samgpt_node_none(model, features, adj, labels, \
                                        sparse, idx_test, shot_num, dataset, 
                                        args.beta, hid_units, nb_classes, unify_dim, num_layers_num,\
                                        args.combinetype, ablation_down, 1, n, 100)
            else: 
                log, acc = adapation.adaptation_samgpt_node_w_acc(model, features, adj, labels, \
                                        sparse, idx_test, shot_num, dataset, 
                                        args.beta, hid_units, nb_classes, unify_dim, num_layers_num,\
                                        args.combinetype, ablation_down, patience, n, 100)

            if n == best_idx: 
                best_acc = acc
                best_model = model 
                torch.save(best_model.state_dict(), save_name+"_best_"+str(best))
            elif n == worst_idx: 
                worst_acc = acc
                worst_model = model 
                torch.save(worst_model.state_dict(), save_name+"_worst_"+str(worst))
            ...
            if acc < worst_acc: 
                ...
                worst_idx = n
                worst_model = model 
                print('WORST MODEL SAVING')
                write(f'worst [{n}]: {idx_perm}')
                torch.save(worst_model.state_dict(), save_name+"_worst_"+str(worst))

        write('=========================================================')
        write(f'Mean ACC: {np.mean(acc_list):.2f} ± {np.std(acc_list):.2f}')
        write(f'Best of {num_permutations}: [{best_idx}] {max(acc_list):.2f}, {best_acc}')
        write(f'Worst of {num_permutations}: [{worst_idx}] {min(acc_list):.2f}, {worst_acc}')
        write('=========================================================')


        center_feature = aug_features[0][0] 

        best_feature = raw_feature[0, :, perm_list[best]]
        worst_feature = raw_feature[0, :, perm_list[worst]]

        features = [center_feature, best_feature, worst_feature]
        feat_mean = [feat.abs().mean(dim=0) for feat in (features)]
        
        best_token = [best_model.feature_prompt_layers[i].weight[0] for i in range(2)]
        worst_token = [worst_model.feature_prompt_layers[i].weight[0] for i in range(2)]
        
        best_token_feat = [best_model.get_token(feat) for feat in [center_feature, best_feature]]
        worst_token_feat = [worst_model.get_token(feat) for feat in [center_feature, worst_feature]]

        print('======BEST=======')
        for data in range(2): 
            print(f'🥟{pretrain_dataset_names[data]}')
            print(f'features mean: {feat_mean[data]}\n')
            print(f'token: {best_token[data]}\n')
            print(f'feat mean * token: {feat_mean[data] * best_token[data]}\n')
            print(f'|feat mean * token|: {(feat_mean[data] * best_token[data]).abs()}\n')
            print(f'AVG(feat*token): {best_token_feat[data][0].abs().mean(dim=0)}\n')
        
        print('\n\n========WORST========')
        for data in [0, 2]: 
            print(f'🥟{pretrain_dataset_names[data//2]}')
            print(f'features mean: {feat_mean[data]}\n')
            print(f'token: {worst_token[data//2]}\n')
            print(f'feat mean * token: {feat_mean[data] * worst_token[data//2]}\n')
            print(f'|feat mean * token|: {(feat_mean[data] * worst_token[data//2]).abs()}\n')
            print(f'AVG(feat*token): {worst_token_feat[data//2][0].abs().mean(dim=0)}\n')

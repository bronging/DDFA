...
from models import LogReg, FeatureMLP, DimensionNN_V2, GCN_encoder, FUG, train_vaes, train_single_vae, train_single_vae_bce, train_vaes_bce
...
parser.add_argument('--stratege_num', type=int, default=0, help='[samgpt, anchor_mlp, vec2mlp, permMDGPT]')
...
def select_feature_indices_by_variance(variances, k, strategy='top'):
    """
    전략에 따라 분산 기반 feature dimension 인덱스 선택
    - variances: [num_features]
    - k: 선택할 차원 수
    - strategy: 'top', 'bottom', 'top_bottom', 'split_three', 'random'
    """
    sorted_indices = torch.argsort(variances)  # ascending (작은 순)

    if strategy == 'bottom':
        return sorted_indices[:k]

    elif strategy == 'top':
        return sorted_indices[-k:]  # 큰 순서

    elif strategy == 'top_bottom':
        half_k = k // 2
        bottom_k = sorted_indices[:half_k]
        top_k = sorted_indices[-(k - half_k):]
        return torch.cat([top_k, bottom_k])

    elif strategy == 'split_three':
        third = k // 3
        remainder = k - (third * 3)  # 부족한 개수 (0, 1, 또는 2)

        bottom = sorted_indices[:third]
        middle = sorted_indices[len(sorted_indices)//2 - third//2 : len(sorted_indices)//2 + (third - third//2)]
        top = sorted_indices[-third:]

        # 나머지 차원은 중간에서 조금 더 가져오기
        if remainder > 0:
            extra = sorted_indices[len(sorted_indices)//2 + (third - third//2) : len(sorted_indices)//2 + (third - third//2) + remainder]
            return torch.cat([top, bottom, middle, extra])
        else:
            return torch.cat([top, bottom, middle])
    
    elif strategy == 'middle':
        mid_start = len(sorted_indices) // 2 - k // 2
        return sorted_indices[mid_start:mid_start + k]

    elif strategy == 'random':
        return torch.randperm(len(variances))[:k]

    elif strategy == 'random_sorted':
        random_indices = torch.randperm(len(variances), device=variances.device)[:k]

        # 해당 인덱스들의 분산 값 기준으로 정렬
        sorted_random = random_indices[torch.argsort(variances[random_indices], descending=True)]
        return sorted_random

    else:
        raise ValueError(f"Unknown strategy: {strategy}")

#                   0        1          2              3          4          5           6
strategy_list = ['bottom', 'top', 'top_bottom', 'split_three', 'random', 'middle', 'random_sorted']
strategy = strategy_list[args.stratege_num] if args.stratege_num != 100 else 'PCA' # 100이면 PCA
write(f'➡️ var strategy: {strategy}')

if model_type == 'samgpt'or model_type =='vae' : 
...
try:
    ...
    if strategy == 'PCA': 
        ...
        # PCA 안하고, 그냥 분산이 큰 raw feature dimension 순서로 정렬하여 50차원만 사용 
        for idx, feat in enumerate(features): 
            variances = feat.var(dim=0)

            selected_indices = select_feature_indices_by_variance(variances, unify_dim, strategy)
            features[idx] = feat[:, selected_indices]
    ...
    if model_type == 'samgpt' or model_type == 'vae'or model_type == 'norm_mdgpt':
...
for data in downstream_loader:
    ...
    # t_feature = torch.FloatTensor(pca_compression(t_feature,k=None)).cuda() # 원래 차원으로 축소 
...
print('Before: ', t_feature.size())
if strategy != 'PCA': 
    variances = t_feature.var(dim=0)        
    selected_indices = select_feature_indices_by_variance(variances, unify_dim, strategy)
    t_feature = t_feature[:, selected_indices]

print('After : ', t_feature.size())
# print('Feature: ', t_feature[0, :50])

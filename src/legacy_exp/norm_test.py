...
def compare_embeddings(emb_org, emb_perm):
    ...
    if emb_org.shape[0] != emb_perm.shape[0]:
        num_nodes = min(emb_org.shape[0], emb_perm.shape[0])
        rng = np.random.RandomState(42)
        idx_a = rng.choice(emb_org.shape[0], num_nodes, replace=False)
        idx_b = rng.choice(emb_perm.shape[0], num_nodes, replace=False)
        emb_org = emb_org[idx_a]
        emb_perm = emb_perm[idx_b]
...
try:
    ...
    if graph_batch and pretrain_method == 'GRAPHCL': 
    ...
    if model_type == 'samgpt' or model_type == 'norm_mdgpt':
        ...
        embed = [model.get_forward(aug_features[i][0], aug_adjs[i][0], sparse, None, LP, i) for i in range(len(aug_features))]

        cos_mat = [[0 for i in range(len(aug_features))] for i in range(len(aug_features))]
        euc_mat = [[0 for i in range(len(aug_features))] for i in range(len(aug_features))]
        for i in range(len(aug_features) ):
            for j in range(i, len(aug_features)):
                print(f'{pretrain_dataset_names[i]} - {pretrain_dataset_names[j]}')
                rst = compare_embeddings(embed[i], embed[j])
                print()
                cos = rst['cosine_mean']
                euc = rst['euclidean_mean']
                cos_mat[i][j] = cos 
                cos_mat[j][i] = cos
        ...
        sim_matrix = np.array(cos_mat)
        plt.figure(figsize=(8, 6))
        n = len(pretrain_dataset_names)
        fig, ax = plt.subplots(figsize=(8, 6))

        cax = ax.imshow(sim_matrix, cmap='coolwarm', vmin=-1, vmax=1)

        # Set axis labels
        ax.set_xticks(np.arange(n))
        ax.set_yticks(np.arange(n))
        ax.set_xticklabels(pretrain_dataset_names, rotation=45, ha='right')
        ax.set_yticklabels(pretrain_dataset_names)

        # Add colorbar
        cbar = fig.colorbar(cax, ax=ax, fraction=0.046, pad=0.04)
        cbar.set_label("Cosine Similarity")

        plt.title("Cosine Similarity Between Graph Embeddings")
        plt.tight_layout()
        plt.savefig(f'{args.model_type}_emb_cosine_sim.png', dpi=300)
        plt.close()

        sim_matrix = np.array(euc_mat)
        plt.figure(figsize=(8, 6))
        n = len(pretrain_dataset_names)
        fig, ax = plt.subplots(figsize=(8, 6))
        ...
        plt.title("Euclidean Distance Between Graph Embeddings")
        plt.tight_layout()
        plt.savefig(f'{args.model_type}_emb_euclidean_dist.png', dpi=300)
        plt.close()
        ...
        #     print(f'AVG(feat*token): {token_feat[data].mean(dim=0)}')
        #     print(f'Embedding: {embed[data].mean(dim=0)}')

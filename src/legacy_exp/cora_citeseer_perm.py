...
try:
    ...
    if model_type == 'samgpt':
        # f_a = aug_features[0][0] # Cora 
        # f_b = aug_features[1][0] # CiteSeer 
        ...
        # num_nodes = min(f_a.shape[0], f_b.shape[0])
        # rng = np.random.RandomState(42)
        # idx_a = rng.choice(f_a.shape[0], num_nodes, replace=False)
        # idx_b = rng.choice(f_b.shape[0], num_nodes, replace=False)
        # f_a_idx = f_a[idx_a]
        # f_b_idx = f_b[idx_b]

        # mmd_rst = torch.zeros((unify_dim, unify_dim), dtype=torch.float32)
        # print(f'f11 [1]: {f_a_idx[:, 0].size()}')
        # print(f'f21 [1]: {f_b_idx[:, 0].size()}')

        # for i in range(unify_dim): 
        #     for j in range(unify_dim): 
        #         # print('a: ', f_a_idx[:, i][:5])
        #         # print('b: ',f_b_idx[:, j][:5])
                
        #         mmd = compute_mmd(f_a_idx[:, i], f_b_idx[:, j], gamma=1.0)
        #         mmd_rst[i, j] = mmd
        #         mmd_rst[j, i] = mmd 
        ...
        # for i in range(unify_dim):
        #     # 이미 매칭된 j는 제외
        #     mmd_vals = mmd_rst[i].clone()
        #     for j in matched_j:
        #         mmd_vals[j] = float('inf')  # 매우 큰 값으로 만들어서 선택되지 않도록
        #     # 가장 작은 MMD 값을 갖는 j 선택
        #     min_j = torch.argmin(mmd_vals).item()
        #     matching.append((i, min_j, mmd_rst[i][min_j].item()))
        #     matched_j.add(min_j)

        # greedy_matching = [] 
        # # 결과 출력
        # for i, j, mmd_val in matching:
        #     # print(f'Cora feature {i} <-> Citeseer feature {j}, MMD={mmd_val:.4f}')
        #     greedy_matching.append(j)
        
        # if isinstance(mmd_rst, torch.Tensor):
        #     cost_matrix = mmd_rst.cpu().numpy()
        # else:
        #     cost_matrix = mmd_rst

        # from scipy.optimize import linear_sum_assignment
        # row_ind, col_ind = linear_sum_assignment(cost_matrix)  # 최소 합이 되는 조합

        # # 결과 출력
        # # for i, j in zip(row_ind, col_ind):
        # #     print(f'Cora feature {i} <-> Citeseer feature {j}, MMD={cost_matrix[i, j]:.4f}')
        
        # print(f'greedy : {greedy_matching}')
        # print(f'헝가리안: {col_ind}')
        # feat = aug_features[1]
        # if args.match == 'greedy': 
        #     write('GREEDY====')
        #     aug_features[1] = feat[:, :, greedy_matching]
        # elif args.match == 'hung': 
        #     write('HUNG====')
        #     aug_features[1] = feat[:, :, col_ind]
        # else: 
        #     write('NONE!====')
        ...
        # print(f'\n=====================================')
        # print(f'✳ Cora-CiteSeer feature {aug_features[0][0].size()}')
        # feature_rst = compare_embeddings(aug_features[0][0], aug_features[-1][0][idx_b])
        # print()
        ...
        # print(f"✳ Cora-CiteSeer embeddings (Before Training) {cora0_emb_before.size()}")
        # before_rst = compare_embeddings(cora0_emb_before, cora1_emb_before[idx_b])
        # print()
        ...
        #     aug_features[0][i] = aug_features[0][i] - aug_features[0][i].min(dim=0).values
        #     aug_features[1][i] = aug_features[1][i] - aug_features[1][i].min(dim=0).values
        # aug_features[0][0] = aug_features[0][0] + aug_features[0][0].abs().mean(dim=0)
        # aug_features[1][0] = aug_features[1][0] + aug_features[1][0].abs().mean(dim=0)
        ...
        # print(f'✳ Cora-CiteSeer token (After Training) {cora0_tokened.size()}')
        # token_rst = compare_embeddings(cora0_tokened, cora1_tokened[idx_b])
        # print()
        ...
        # print(f'✳ Cora-CiteSeer embeddings (After Training) {cora0_emb_after.size()}')
        # after_rst = compare_embeddings(cora0_emb_after, cora1_emb_after[idx_b])
        # print()
        ...
        # print(f'✳ Cora-CiteSeer embeddings w/o token! (After Training) {cora0_wotoken.size()}')
        # rst = compare_embeddings(cora0_wotoken, cora1_wotoken[idx_b])
        # print()

        # print(f'======================================')
        # print(f'✳ Cora-CiteSeer token (After Training) (node0)')
        # print(f'cora: {cora0_tokened[0]}')
        # print(f'cite: {cora1_tokened[0]}')
        ...
        #     print('cora0*t|', end='')
        #     for i in range(10*j, 10*j+10):
        #         print(f'{cora0_feat_mean[i]*cora0_token[i].abs().item():8.4f}', end='')
        #     print()
            
        #     print('cora1*t|', end='')
        #     for i in range(10*j, 10*j+10):
        #         print(f'{cora1_feat_mean[i]*cora1_token[i].abs().item():8.4f}', end='')
        #     print('\n\n')

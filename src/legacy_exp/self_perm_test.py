...
parser.add_argument('--perm', type=str2bool, default=True, help='Cora1 permutation? ')
parser.add_argument('--numCora', type=int, default=2, help='Only Cora0 or Cora0 & Cora1')
...
if args.numCora == 2: 
    pretrain_dataset_names = ['Cora', 'Cora'] 
elif args.numCora == 1: 
    pretrain_dataset_names = ['Cora']
else: 
    pretrain_dataset_names = get_pretrain_dataset_names(data, source_id, target_id)
...
try:
    ...
    print(f'feature len: {len(aug_features)}')
    print(f'Feature size: {aug_features[0].size()}')
    ...
    if model_type == 'samgpt':
        ...
        # 1. permutation 
        if args.perm and args.numCora == 2:
            idx_perm = np.random.permutation(aug_features[1].shape[-1]) # feat dim = 50 
            perm_feat = aug_features[1]
            aug_features[1] = perm_feat[:, :, idx_perm]
            
            print(f'[Perm]: {idx_perm}')
        elif args.perm and args.numCora == 1: 
            num_node = aug_features[0].size(1)
            idx_perm = np.random.permutation(aug_features[0].shape[-1]) # feat dim = 50 

            print(f'Before Perm: {aug_features[0][0][-1]}')
            perm_feat = aug_features[0]
            perm_feat[:, num_node//2:] = perm_feat[:, num_node//2:, idx_perm]
            
            aug_features[0] = perm_feat

            print(f'[Perm]: {idx_perm}')
            print(f'After Perm: {aug_features[0][0][-1]}')

        if args.numCora < 3:
            print(f'cora0 col mean: {aug_features[0][0].mean(0).size()} {aug_features[0][0].mean(0)}\n')
            
            print(f'Dataset 0 (node0): {aug_features[0][0][0]}')
            print(f'Dataset 1 (node0): {aug_features[-1][0][0]}')

            print(f'\n=====================================')
            print(f'✳ Cora0-Cora1 feature {aug_features[0][0].size()}')
            feature_rst = compare_embeddings(aug_features[0][0], aug_features[-1][0])
            print()

            cora0_emb_before = model.embed(model.get_token(aug_features)[0], aug_adjs[0][0], sparse, None, LP)[0].squeeze(0)
            # print(f'emb size: {cora0_emb_before.size()}') # [2708, 256]
            cora1_emb_before = model.embed(model.get_token(aug_features)[1], aug_adjs[1][0], sparse, None, LP)[0].squeeze(0)

            print(f"✳ Cora0-Cora1 embeddings (Before Training) {cora0_emb_before.size()}")
            before_rst = compare_embeddings(cora0_emb_before, cora1_emb_before)
            print()

            cora0_wotoken = model.embed(aug_features[0][0], aug_adjs[0][0], sparse, None, LP)[0].squeeze(0)
            cora1_wotoken = model.embed(aug_features[-1][0], aug_adjs[-1][0], sparse, None, LP)[0].squeeze(0)
            print(f'✳ Cora0-Cora1 embeddings w/o token! (After Training) {cora0_wotoken.size()}')
            rst = compare_embeddings(cora0_wotoken, cora1_wotoken)
            print()

        # for i in range(4): 
        #     aug_features[0][i] = aug_features[0][i] + aug_features[0][i].abs().mean(dim=0)
        #     aug_features[1][i] = aug_features[1][i] + aug_features[1][i].abs().mean(dim=0)
        train.train_samgpt_graphcl(model=model, lr=lr, weight_decay=l2_coef, 
                        start_epoch=args.restart_epoch, num_epoch=200, 
                        aug_features=aug_features, aug_adjs=aug_adjs, 
                        lbls=lbls, sparse=sparse, save_name=save_name, patience=patience)
        
        # print(f'cora0 - node 0 +++++ \n{model.embed(aug_features[0][0], aug_adjs[0][0], sparse, None, LP)[0][0]}\n')
        # print(f'cora1 - node 0 +++++ \n{model.embed(aug_features[-1][0], aug_adjs[0][0], sparse, None, LP)[0][0]}\n')
        
        arr = [0, 1]
        for node_id in arr: 
            for data in range(len(aug_features)): 
                print(f'{pretrain_dataset_names[data]} - node {node_id} @@@@@')
                print(f'Before token: {aug_features[data][0][node_id]}')
                print(f'After token : {model.get_token(aug_features)[data][node_id]}\n')
        
        if args.numCora < 3: 
            print(f'💫 feature mean! ')
            print(f'cora0: {aug_features[0][0].mean(dim=0)}')
            print(f'cora1: {aug_features[1][0].mean(dim=0)}')

            print(f'cora0 - ABS: {aug_features[0][0].abs().mean(dim=0)}')
            print(f'cora1 - ABS: {aug_features[1][0].abs().mean(dim=0)}')

            print(f'cora0 - token: {model.get_token(aug_features)[0].mean(dim=0)}')
            print(f'cora1 - token: {model.get_token(aug_features)[1].mean(dim=0)}')


            print(f'\n=====================================')
            cora0_tokened = model.get_token(aug_features)[0]
            cora1_tokened = model.get_token(aug_features)[-1]
            print(f'✳ Cora0-Cora1 token (After Training) {cora0_tokened.size()}')
            token_rst = compare_embeddings(cora0_tokened, cora1_tokened)
            print()

            cora0_emb_after = model.embed(model.get_token(aug_features)[0], aug_adjs[0][0], sparse, None, LP)[0].squeeze(0)
            cora1_emb_after = model.embed(model.get_token(aug_features)[-1], aug_adjs[-1][0], sparse, None, LP)[0].squeeze(0)
            # print(f'[embed] {cora1_emb_after}')
            print(f'✳ Cora0-Cora1 embeddings (After Training) {cora0_emb_after.size()}')
            after_rst = compare_embeddings(cora0_emb_after, cora1_emb_after)
            print()
        ...
        print(f'✳ ✳ ✳ Cora0-Cora1 feature Mean, domain token {aug_features[0].mean(0).size()}')
        # cora0_feat_mean = aug_features[0][0].abs().mean(dim=0) 
        # cora1_feat_mean = aug_features[1][0].abs().mean(dim=0)
        # cora0_token = model.feature_prompt_layers[0].weight[0]
        # cora1_token = model.feature_prompt_layers[1].weight[0]


        feat_mean = [aug_features[i][0].abs().mean(dim=0) for i in range(len(aug_features))]
        token = [model.feature_prompt_layers[i].weight[0] for i in range(len(aug_features))] 
        token_feat = model.get_token(aug_features)
        print(f"####token feat: {token_feat[0].size()}")
        print(f'Raw feat')
        print(f'{aug_features[0][0][0]}')
        print(f'Token feat')
        print(f'{token_feat[0][0]}')

        # print(f'token: {cora0_token.size()}')
        for j in range(5):
            print('           ', end='')
            ...
            # print('✳cora0-f', end='')
            # for i in range(10*j, 10*j+10):
            #     print(f'{cora0_feat_mean[i].item():8.4f}', end='')
            # print()
            # print('✳cora1-f', end='')
            # for i in range(10*j, 10*j+10):
            #     print(f'{cora1_feat_mean[i].item():8.4f}', end='')
            # print('\n')

            for data in range(len(aug_features)): 
                print(f'{pretrain_dataset_names[data][:8]:8}-f', end='')
                for i in range(10*j, 10*j+10): 
                    print(f'{feat_mean[data][i].item():-8.4f}', end='')
                print()
            print()

            # print('✳cora0-t', end='')
            # for i in range(10*j, 10*j+10):
            #     print(f'{cora0_token[i].item():8.4f}', end='')
            # print()
            
            # print('✳cora1-t', end='')
            # for i in range(10*j, 10*j+10):
            #     print(f'{cora1_token[i].item():8.4f}', end='')
            # print('\n')

            for data in range(len(aug_features)): 
                print(f'{pretrain_dataset_names[data][:8]:8}-t', end='')
                for i in range(10*j, 10*j+10): 
                    print(f'{token[data][i].item():8.4f}', end='')
                print()
            print()

            # print('✳cora0*t', end='')
            # for i in range(10*j, 10*j+10):
            #     print(f'{cora0_feat_mean[i]*cora0_token[i].item():8.4f}', end='')
            # print()
            
            # print('✳cora1*t', end='')
            # for i in range(10*j, 10*j+10):
            #     print(f'{cora1_feat_mean[i]*cora1_token[i].item():8.4f}', end='')
            # print('\n\n')

            for data in range(len(aug_features)): 
                print(f'{pretrain_dataset_names[data][:8]:8}*t', end='')
                for i in range(10*j, 10*j+10): 
                    print(f'{feat_mean[data][i] * token[data][i].item():8.4f}', end='')
                print()
            print()
            ...
            for data in range(len(aug_features)): 
                print(f'{pretrain_dataset_names[data][:8]:8}*t|', end='')
                for i in range(10*j, 10*j+10): 
                    print(f'{(feat_mean[data][i] * token[data][i]).abs().item():8.4f}', end='')
                print()
            print()

            for data in range(len(aug_features)): 
                print(f'{pretrain_dataset_names[data][:8]:8}*t avg', end='')
                for i in range(10*j, 10*j+10): 
                    print(f'{token_feat[data].abs().mean(dim=0)[i].item():8.4f}', end='')
                print()
            print()
        
        for data in range(len(aug_features)): 
            print(f'🥟{pretrain_dataset_names[data]}')
            print(f'features mean: {feat_mean[data]}\n')
            print(f'token: {token[data]}\n')
            print(f'feat mean * token: {feat_mean[data] * token[data]}')
            print(f'|feat mean * token|: {(feat_mean[data] * token[data]).abs()}')
            print(f'AVG(feat*token): {token_feat[data].abs().mean(dim=0)}')
